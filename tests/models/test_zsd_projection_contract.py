...
def _zsd_training_frame() -> pd.DataFrame:
    return pd.DataFrame(
        [
            {
                "date": "2025-10-25",
...
def test_zsd_projection_includes_projected_scores() -> None:
    model = ZSDModel(random_seed=7)
    model.fit(_zsd_training_frame())
...
def test_zsd_prediction_validates_without_total_inconsistent() -> None:
    model = ZSDModel(random_seed=7)
    model.fit(_zsd_training_frame())
    predictions = model.predict(_upcoming_frame())
