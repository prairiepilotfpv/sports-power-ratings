...
FIXTURE_PATH = Path(__file__).resolve().parents[1] / "fixtures" / "calendar_projection" / "schedule_output.csv"
...
def _assert_numeric_series(series: pd.Series) -> None:
    ...
    assert numeric.notna().sum() == series.notna().sum(), "Found non-numeric values in numeric column"
...
def test_calendar_projection_output_schema_and_types() -> None:
    ...
    for column in string_columns:
        ...
        assert values.map(lambda value: isinstance(value, str)).all(), f"{column} should be string-like"
...
def test_final_results_match_scores() -> None:
    df = pd.read_csv(FIXTURE_PATH)
    final_rows = df[(df["status"] == "final") & df["home_score"].notna() & df["away_score"].notna()]
...
def test_projected_scores_sum_to_total() -> None:
    df = pd.read_csv(FIXTURE_PATH)
    with_scores = df[df["projected_home_score"].notna() & df["projected_away_score"].notna()]
...
def test_projected_spread_sign_convention() -> None:
    ...
    for _, row in with_scores.iterrows():
        ...
        assert math.isclose(row["projected_home_spread"], margin, rel_tol=1e-6, abs_tol=0.5)
